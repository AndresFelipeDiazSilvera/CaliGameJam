...
public class PlaneControl : MonoBehaviour
{
    ...
    private void Update()
    {
        // Verificamos la entrada t�ctil o el mouse             
        HandleTouchInput();
        HandleMouseInput();
    }
    ...
    {
        if (Input.GetMouseButtonDown(0)) // Si el bot�n izquierdo del mouse acaba de ser presionado        
            DragStart(Input.mousePosition);

        // No hay un "MousePhase.Moved" expl�cito, la posici�n del mouse se actualiza continuamente
        // Asumimos que si el bot�n est� presionado, estamos "arrastrando"
        if (Input.GetMouseButton(0))
            Dragging(Input.mousePosition);


        if (Input.GetMouseButtonUp(0)) // Si el bot�n izquierdo del mouse acaba de ser soltado        
